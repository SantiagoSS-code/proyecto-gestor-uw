...
async function POST(request) {
    try {
        ...
        if ("TURBOPACK compile-time truthy", 1) {
            const adminProjectId = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$firebase$2f$admin$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["adminAuth"].app?.options?.projectId || "unknown";
            console.log(`[AuthSession] adminProjectId=${adminProjectId} token.aud=${payload?.aud || "unknown"} token.uid=${payload?.sub || ""} token.email=${payload?.email || ""}`);
        }
        // Verify first so we don't set junk tokens.
        let verificationError = null;
        try {
            await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$firebase$2f$admin$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["adminAuth"].verifyIdToken(idToken);
        } catch (err) {
            verificationError = err;
        }
        const allowUnverifiedInDev = ("TURBOPACK compile-time value", "development") === "development";
        if (verificationError && !allowUnverifiedInDev) {
            throw verificationError;
        }
        if (verificationError && allowUnverifiedInDev) {
            console.warn(`[AuthSession] Skipping token verification in development: ${verificationError?.message || verificationError}`);
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f289a1f2._.js.map